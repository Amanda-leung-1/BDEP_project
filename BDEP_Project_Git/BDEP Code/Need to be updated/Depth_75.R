...
depth_75 <- depth_75 %>%
  mutate(SM1_depth_75 = if_else(Date_depth_75 %in% SM1_depth_75$Date_SM1, SM1_depth_75$SM1_Depth75[match(Date_depth_75, SM1_depth_75$Date_SM1)], NA_real_))
depth_75 <- depth_75 %>%
  mutate(SM2_depth_75 = if_else(Date_depth_75 %in% SM2_depth_75$Date_SM2, SM2_depth_75$SM2_Depth75[match(Date_depth_75, SM2_depth_75$Date_SM2)], NA_real_))
depth_75 <- depth_75 %>%
  mutate(SM3_depth_75 = if_else(Date_depth_75 %in% SM3_depth_75$Date_SM3, SM3_depth_75$SM3_Depth75[match(Date_depth_75, SM3_depth_75$Date_SM3)], NA_real_))
depth_75 <- depth_75 %>%
  mutate(SM4_depth_75 = if_else(Date_depth_75 %in% SM4_depth_75$Date_SM4, SM4_depth_75$SM4_Depth75[match(Date_depth_75, SM4_depth_75$Date_SM4)], NA_real_))
depth_75 <- depth_75 %>%
  mutate(SM5_depth_75 = if_else(Date_depth_75 %in% SM5_depth_75$Date_SM5, SM5_depth_75$SM5_Depth75[match(Date_depth_75, SM5_depth_75$Date_SM5)], NA_real_))
depth_75 <- depth_75 %>%
  mutate(SM6_depth_75 = if_else(Date_depth_75 %in% SM6_depth_75$Date_SM6, SM6_depth_75$SM6_Depth75[match(Date_depth_75, SM6_depth_75$Date_SM6)], NA_real_))


# Convert to xts object for plotting
depth_75_xts <- xts(
  x = depth_75[, c("SM1_depth_75", "SM2_depth_75", "SM3_depth_75", "SM4_depth_75", "SM5_depth_75", "SM6_depth_75")],
  order.by = depth_75$Date_depth_75
)

# Create a list of dygraphs objects
plotobj_75 <- list(
  dygraphs::dygraph(Precipitation_xts, group="Depth_75", main="Soil Moisture sensors at depth = 75cm", height = 200 )%>%
  ...
  dygraphs::dygraph(depth_75_xts, group="Depth_75", main=NULL)%>%
...
htmltools::browsable(htmltools::tagList(plotobj_75))




depth_75_long <- tidyr::pivot_longer(depth_75, 
                                     cols = starts_with("SM"), 
                                     names_to = "Sensor", 
                                     values_to = "Moisture_Content")
# Plot using ggplot2
depth_75_plot <- ggplot(depth_75_long, aes(x = Date_depth_75, y = Moisture_Content, color = Sensor)) +
  geom_line() +
  labs(x = "Date", y = "Soil Moisture Index", title = "Soil Moisture Index for all sensors at a depth of 75 cm") +
