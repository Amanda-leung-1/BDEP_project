...
create_dygraph <- function(gauge_name) {
  ...
  file_save <- sprintf("Files/SMMR_Comparison_Results/%s.html", gauge_name)
...
gauge_info <- data.frame(name_all, latitude = c(
  48.79883333, 48.83305556, 49.01140833, 49.00751667,
  48.96352778, 48.97481944, 48.57886667, 49.00566667,
  49.00155, 48.36363333, 48.42031111, 48.45270278),
longitude = c(
  -113.6567, -113.4197, -113.2995, -112.5453,
  -113.0624, -110.4218, -109.3911, -109.7178,
  -109.4225, -108.3562, -107.1711, -107.0365)) 

## Importing HYPE Data 
file_directory <- "Files/HYPE_V2" # Define the directory containing the files
...
#RAVEN DATA IMPORT 
RAVEN_Data_stmary <- read.csv("Files/hydrographs_stmary_raven.csv", header = TRUE)
RAVEN_Data_milk <- read.csv("Files/hydrographs_milk_raven.csv", header = TRUE)
raven_names <- read.csv("Files/raven_names.csv", header = TRUE)
...
#calculate KGE, NSE, and log NSE with these timesteps 
