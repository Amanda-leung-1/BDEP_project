...
process_time_series <- function(gauge_names, RAVEN_Data_All, HYPE_Data_All, PRMS_Data_All, Obs_flow) {
  ...
  timeseries_full_list <- list()  # To store full time series statistics
  
  # Loop through each gauge name
  for (gauge_name in gauge_names) {
    ...
    timeseries_weekly <- timeseries_day %>%
      ...
      ungroup() %>%
      select(-week_start)  # Optionally remove week_start if not needed
    ...
    # Store full time series statistics
  ...
  return(list(
   ...
   seasonal = timeseries_seasonal_list,
    full = timeseries_full_list  # Full time series statistics
  ))
}
